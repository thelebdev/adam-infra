...
- `test_claude_session.sh` — the `claude-session` helper: session-name and
  user-identity validation, and workspace-directory confinement (the
  `..`/symlink/absolute-path escape rejections). Directory tests need GNU
  `realpath -m` and self-skip on non-Linux hosts.
- `test_session_manager.py` — the `session-manager` API: directory
  confinement, `tmux` output parsing, error mapping, and the `Remote-User`
  trust boundary (ambiguous/malformed identities are refused).
