...
checks across SSH / firewall / sudoers / file-integrity / cert-expiry /
auth-log / container-runtime, writes a structured JSON snapshot to
`/var/log/infra/security-posture.json`, and surfaces traffic-light
status on a new dashboard panel. Drift between snapshots (e.g.
`PasswordAuthentication` flipped to `yes`) raises severity and emits
an alert via the (also-pending) Alertmanager wiring.

Checks in scope for v1:

- `ufw status verbose` — active + expected rule set
- `fail2ban-client status` — active + jail counts; recent bans
- sshd config drift — `PermitRootLogin no`, `PasswordAuthentication no`,
  `MaxAuthTries 3` still in place
- `/etc/passwd` + `/etc/sudoers.d/` diff against a baseline snapshot
- Listening sockets (`ss -tlnp`) vs an expected allowlist
- `unattended-upgrades` last-run age (alert if > 36h)
- Caddy TLS cert expiry per subdomain (alert if < 14 days)
- Container image age + known-CVE flags via Trivy (best-effort)
- SUID-binary inventory diff against a baseline
- Recent SSH ban + Authelia auth-failure counts (last 24h)

Implementation: small Python script + systemd timer in
`platform/security/posture/`, no new daemon. The dashboard already
polls session-manager's `/api/version`; add `/api/security` next to
it. Baseline files live under `platform/security/baselines/` and are
regenerated by an opt-in `recompute-baseline` command after the
operator has approved current state.
...
scripted suite under `security/probes/` for the tests that don't need
human judgment. Run on demand against any `infra` deployment; output a
report listing which expected protections held and which didn't.

Test families:

1. **External recon** — nmap from a remote IP, expecting only 22 / 80 /
   443 plus the documented Authelia + Caddy services
2. **TLS posture** — `testssl.sh` and / or SSL Labs API against every
   subdomain, score >= A
3. **HTTP attack surface** — `nuclei` templates against each subdomain;
   OWASP ZAP baseline scan
4. **Auth gates** — direct calls to `/api/sessions`, `grafana`,
   `dashboard` without an Authelia session expecting 302 → portal
   (never 200)
5. **Brute-force shape** — N bogus TOTPs against Authelia and N bad
   SSH passwords expecting rate-limiting / fail2ban bans (run from a
   throwaway IP so the operator's home doesn't get locked out)
6. **Container surface** — common escape probes from inside each
   running container: mounted sockets, dangerous capabilities,
   `runc` CVE checks
7. **Host privesc** — `linpeas` / `linenum` as the admin user,
   triaging anything actionable
8. **Secret leakage** — grep container env, process env, log lines
   for anything matching password / token / API-key shape
9. **Security headers** — confirm Caddy responses include `HSTS`,
   `CSP`, `X-Frame-Options`, `X-Content-Type-Options` on every gated
   route

Each probe lands as a small Python or bash script under
`security/probes/<family>/<name>`; the runbook glues them into named
test plans (`smoke` — 5 min; `full` — 30 min). Each probe writes
result + evidence to `security/last-run/<timestamp>/`.

The posture monitor and Security QA share a vocab: the monitor checks
state continuously; the runbook attacks the state to confirm the
